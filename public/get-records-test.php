...
// Test mit GET records statt POST _find
...
// Test-Layouts
$layouts = ['sym_Banner', 'sym_Module'];

// Teste GET records (alle Records) statt POST _find
foreach ($layouts as $layout) {
    $records_url = $host.'/fmi/data/vLatest/databases/'.$db.'/layouts/'.$layout.'/records?_limit=5';
    ...
    if ($response) {
        ...
        if ('0' === $testResult['fm_code']) {
            ...
            if (!empty($data['response']['data'])) {
                ...
                $testResult['recordId'] = $sampleRecord['recordId'] ?? 'N/A';
        ...
        if ($httpCode >= 400) {
            $testResult['response_preview'] = substr($response, 0, 300);
        }
    } else {
