...
class SymOrdersItemsDebugController extends AbstractController
{
    public function __construct(private FileMakerLayoutRegistry $layouts)
    {
    }

    #[Route('/sym_orders_items-json', name: 'sym_orders_items_json')]
    public function index(FileMakerClient $fm, LoggerInterface $logger): JsonResponse
    {
        try {
            $layout = $this->layouts->get('orders_items', 'sym_Orders_items');
            ...
            $fmLogger->error('sym_Orders_items debug error: '.$e->getMessage(), ['exception' => $e, 'layout' => $layout]);
