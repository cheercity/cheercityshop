...
class SymArtikelItemsImagesDebugController extends AbstractController
{
    public function __construct(private FileMakerLayoutRegistry $layouts)
    {
    }

    #[Route('/sym_artikel_items_images-json', name: 'sym_artikel_items_images_json')]
    public function index(FileMakerClient $fm, LoggerInterface $logger): JsonResponse
    {
        try {
            $layout = $this->layouts->get('artikel_items_images', 'sym_Artikel_items_images');
            ...
            $fmLogger->error('sym_Artikel_items_images debug error: '.$e->getMessage(), ['exception' => $e, 'layout' => $layout]);
